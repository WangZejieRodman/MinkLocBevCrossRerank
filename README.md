...
* `eval/evaluate_cyd_JointScore.py`: 在第二阶段重排序时，不只按 DTW 距离排序,而是构建一个联合得分 (Joint Score)：`Final_Distance=β⋅DistanceBEV​+(1−β)⋅DistanceDTW`,让 BEV 的全局特征差异和 Cross 的局部序列差异共同投票，压制“长得像但不在一个路口”的假阳性。
