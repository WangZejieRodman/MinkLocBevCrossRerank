# eval/evaluate_cyd_JointScore.py
import os
import sys
import pickle
import numpy as np
...
def evaluate_cyd(model, device, params: TrainingParams, log=False, show_progress=False, fusion_weight=0.5):
    ...
    print(f"\nStarting Coarse-to-Fine Dual-Stream Retrieval (Late Fusion Weight: {fusion_weight})...")
    for j, query_set in enumerate(tqdm.tqdm(query_sets, desc="Retrieving")):
        ...
        for query_idx in range(len(q_globals)):
            ...
            # 1. 第一阶段粗检索 (记录粗检索距离 coarse_distances)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("开始评估 CYD 数据集 (混合双流 + Late Fusion)")
    print("=" * 60)

    # 这里的 fusion_weight 可调，0.5 表示 Coarse 和 Fine 对半开。如果想偏向 BEV 可以设为 0.7。
    stats = evaluate_cyd(model, device, params, show_progress=True, fusion_weight=0.5)
    ...
    if stats is not None:
        ...
        print("-" * 60)
        print("【第二阶段：精流 (Late Fusion Re-ranking)】")
